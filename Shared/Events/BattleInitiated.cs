...
public class BattleInitiated : GameEvent
{
    ...
    {
        return g.CurrentJuice != null && g.CurrentJuice.Type == JuiceType.GoFirst &&
               g.CurrentJuice.Player == initiator && PlayerSequence.IsAfter(g, initiator, target);
    }

    public static bool IsTargetByJuice(Game g, Player initiator, Player target)
    {
        return g.CurrentJuice != null && g.CurrentJuice.Type == JuiceType.GoLast &&
               g.CurrentJuice.Player == initiator && PlayerSequence.IsAfter(g, initiator, target);
    }
    ...
    public TreacheryCalled TreacheryOf(Player p)
    {
        ...
        if (Game.DefenderTraitorAction != null && (Game.DefenderTraitorAction.Initiator == p.Faction || Game.DefenderTraitorAction.Initiator == p.Ally))
