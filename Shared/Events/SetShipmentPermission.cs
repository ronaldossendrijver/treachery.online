...
public class SetShipmentPermission : GameEvent
{
    ...
    {
        return g.CurrentMainPhase == MainPhase.ShipmentAndMove && p.Is(Faction.Orange) && p.HasHighThreshold();
    }
    ...
    protected override void ExecuteConcreteEvent()
    {
        ...
        foreach (var f in Factions) Game.ShipmentPermissions.Remove(f);

        if (Permission != ShipmentPermission.None)
            foreach (var f in Factions) Game.ShipmentPermissions.Add(f, Permission);
    }
