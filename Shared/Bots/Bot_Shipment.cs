    ...
    public partial class Player
    {
        ...
        protected virtual void DetermineShipment_ShipDirectlyToSpiceOrDiscoveryAsOrangeOrOrangeAlly()
        {
            ...
            if (bestLocation != null)
            {
                ...
                if (DetermineShortageForShipment(forcesNeededForCollection, false, bestLocation, Faction.None, ForcesInReserve, 0, out int nrOfForces, out int nrOfSpecialForces, out int noFieldValue, out int cunningNoFieldValue, 0, 99, false) <= 0)
                {
            ...
            if (bestLocation != null)
            {
                ...
                if (DetermineShortageForShipment(forcesNeeded, false, bestLocation, Faction.None, ForcesInReserve, 0, out int nrOfForces, out int nrOfSpecialForces, out int noFieldValue, out int cunningNoFieldValue, 0, 99, false) <= 0)
                {
                    DoShipment(ShipmentDecision.AtResources, nrOfForces, nrOfSpecialForces, noFieldValue, cunningNoFieldValue, bestLocation, true, true);
                    return;
                }
        ...
        protected virtual void DetermineShipment_BackToReserves()
        {
            ...
            {
                DoShipment(ShipmentDecision.BackToReserves, -battaltionToEvacuate.Value.AmountOfForces, -battaltionToEvacuate.Value.AmountOfSpecialForces, -1, -1, battaltionToEvacuate.Key, false, true);
            }
        }
        ...
        protected virtual void DetermineShipment_ShipDirectlyToSpiceOrDiscoveryAsYellow()
        {
            ...
            if (bestLocation != null)
            {
                ...
                if (DetermineShortageForShipment(forcesToRally, false, bestLocation, Faction.None, ForcesInReserve, SpecialForcesInReserve, out int nrOfForces, out int nrOfSpecialForces, out int noFieldValue, out int cunningNoFieldValue, 0, 99, false) <= 3)
                {
        ...
        protected virtual void DetermineShipment_ShipToStrongholdNearSpice()
        {
            ...
            if (richestLocation != null)
            {
                ...
                if (locationToShipTo != null && AnyForcesIn(locationToShipTo) < 10)
                {
                    ...
                    DetermineShortageForShipment(forcesNeededForCollection + TotalMaxDialOfOpponents(richestLocation.Territory), false, locationToShipTo, opponentFaction, ForcesInReserve, SpecialForcesInReserve, out int nrOfForces, out int nrOfSpecialForces, out int noFieldValue, out int cunningNoFieldValue, 0, 99, Faction == Faction.Grey);
        ...
        protected virtual void DetermineShipment_StrengthenWeakestStronghold(bool onlyIfThreatened, int extraForces, float shortageToAccept, bool takeReinforcementsIntoAccount)
        {
            ...
            if (weakestStronghold != null)
            {
                ...
                if (DetermineShortageForShipment(dialNeeded, true, weakestStronghold.Location, opponentFaction, ForcesInReserve, SpecialForcesInReserve, out int nrOfForces, out int nrOfSpecialForces, out int noFieldValue, out int cunningNoFieldValue, 0, 0, true) <= shortageToAccept)
                {
        ...
        protected virtual void DetermineShipment_DummyAttack(int minResourcesToKeep)
        {
            ...
            if (targetOfDummyAttack != null)
            {
                ...
                else if (DetermineShortageForShipment(0.5f, true, targetOfDummyAttack, OccupyingOpponentIn(targetOfDummyAttack.Territory).Faction, ForcesInReserve, SpecialForcesInReserve, out int nrOfForces, out int nrOfSpecialForces, out int noFieldValue, out int cunningNoFieldValue, minResourcesToKeep, 1, false) <= 0)
                {
        ...
        protected virtual void DetermineShipment_UnlockMoveBonus(int minResourcesToKeep)
        {
            if (!Game.HasOrnithopters(this) && AnyForcesIn(Game.Map.PolarSink) > 0)
            {
                ...
                if (target != null)
                {
                    ...
                    if (DetermineShortageForShipment(needed, true, target, opponent != null ? opponent.Faction : Faction.None, ForcesInReserve, SpecialForcesInReserve, out int nrOfForces, out int nrOfSpecialForces, out int noFieldValue, out int cunningNoFieldValue, minResourcesToKeep, 1, false) <= 0)
                    {
                        DoShipment(ShipmentDecision.DummyShipment, nrOfForces, nrOfSpecialForces, noFieldValue, cunningNoFieldValue, target, false, true);
                    }
                }
            }
        }
        ...
        private bool VacantAndValid(Location location) => VacantAndSafeFromStorm(location) && ValidShipmentLocations(false).Where(l => l.IsStronghold).Contains(location);
        ...
        protected virtual void DetermineShipment_PreventFremenWin()
        {
            ...
            if (Game.IsPlaying(Faction.Yellow) && Game.YellowVictoryConditionMet)
            {
                ...
                {
                    DetermineShortageForShipment(99, true, Game.Map.HabbanyaSietch, Faction.None, ForcesInReserve, SpecialForcesInReserve, out int nrOfForces, out int nrOfSpecialForces, out int noFieldValue, out int cunningNoFieldValue, 0, 99, true);
                ...
                {
                    DetermineShortageForShipment(99, true, Game.Map.SietchTabr, Faction.None, ForcesInReserve, SpecialForcesInReserve, out int nrOfForces, out int nrOfSpecialForces, out int noFieldValue, out int cunningNoFieldValue, 0, 99, true);
        ...
        private Attack ConstructAttack(Location location, int extraForces, int minResourcesToKeep, int maxUnsupportedForces)
        {
            ...
            if (opponent != null)
            {
                ...
                var shortageForShipment = DetermineShortageForShipment(Math.Max(dialNeeded, 0.5f) + extraForces, true, location, opponent.Faction, ForcesInReserve, SpecialForcesInReserve, out int forcesToShip, out int specialForcesToShip, out int noFieldValue, out int cunningNoFieldValue, minResourcesToKeep, maxUnsupportedForces, !RedVersusYellow(opponent));
